...
from src.domain.schemas import (
    ...
    PostCreate, PostListItem, PostOut, PostUpdate,
    TagCreate, TagOut,
)
...
# ── Auth ─────────────────────────────────────────────
@router.post("/login", response_model=TokenOut, summary="管理员登录", description="使用用户名和密码登录，返回 JWT Token。")
...
async def dashboard(db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    ...
    return {
        ...
        "recent_posts": [{"id": p.id, "title": p.title, "status": p.status, "created_at": p.created_at.isoformat()} for p in recent_posts],
        "recent_comments": [
            {"id": c.id, "author_name": c.author_name, "content": c.content[:50], "is_approved": c.is_approved, "created_at": c.created_at.isoformat()}
...
async def admin_list_posts(
    ...
    posts, total = await post_service.get_posts(db, page=page, page_size=page_size, status=status, keyword=q)
    items = []
    for p in posts:
        items.append({
...
@router.post("/posts")
async def admin_create_post(data: PostCreate, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    post = await post_service.create_post(db, data)
    return {"id": post.id, "slug": post.slug}


@router.get("/posts/{post_id}")
async def admin_get_post(post_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    ...
    return {
        ...
        "is_top": post.is_top, "is_allow_comment": post.is_allow_comment,
        "views": post.views, "category_id": post.category_id,
...
async def admin_update_post(post_id: int, data: PostUpdate, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
async def admin_delete_post(post_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
async def admin_list_categories(db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    cats = (await db.execute(select(Category).order_by(Category.name))).scalars().all()
    result = []
    for c in cats:
        count = (await db.execute(
            select(func.count()).select_from(Post).where(Post.category_id == c.id)
        )).scalar() or 0
...
async def admin_create_category(data: CategoryCreate, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
async def admin_update_category(cat_id: int, data: CategoryCreate, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
async def admin_delete_category(cat_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    cat = await db.get(Category, cat_id)
    if not cat:
        raise HTTPException(status_code=404, detail="Category not found")
    await db.delete(cat)
...
# ── Tags CRUD ────────────────────────────────────────
@router.get("/tags")
async def admin_list_tags(db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    tags = (await db.execute(select(Tag).order_by(Tag.name))).scalars().all()
    return [{"id": t.id, "name": t.name, "slug": t.slug} for t in tags]


@router.post("/tags")
async def admin_create_tag(data: TagCreate, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
async def admin_delete_tag(tag_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
async def admin_approve_comment(comment_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
async def admin_delete_comment(comment_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
# ── Links CRUD ───────────────────────────────────────
@router.get("/links")
async def admin_list_links(db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    links = (await db.execute(select(Link).order_by(Link.sort_order))).scalars().all()
    return [{"id": l.id, "name": l.name, "url": l.url, "description": l.description, "sort_order": l.sort_order, "is_visible": l.is_visible} for l in links]


@router.post("/links")
async def admin_create_link(data: LinkCreate, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    link = Link(name=data.name, url=data.url, description=data.description, sort_order=data.sort_order, is_visible=data.is_visible)
    db.add(link)
    await db.commit()
...
async def admin_delete_link(link_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
...
# ── Settings ─────────────────────────────────────────
@router.get("/settings")
async def admin_get_settings(db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
    settings = (await db.execute(select(Setting))).scalars().all()
    return {s.key: s.value for s in settings}


@router.put("/settings")
async def admin_update_settings(data: dict, db: AsyncSession = Depends(get_db), _: User = Depends(auth.get_current_user)):
